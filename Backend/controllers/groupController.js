...
const makeNewGroup = async (req, res, next) => {
  try {
    ...
    if (!groupName || !ownerId) return next(new ApiError('No group name or owner', 400))
    if (await groupNameAlreadyInUse(groupName))
      return next(new ApiError('Group name Already in use', 400))
...
const addMovieForGroup = async (req, res, next) => {
  try {
    ...
    if (result === 'Movie already in group')
      return next(new ApiError('Movie already in group', 400))
...
/**
 * Lisää näytös ryhmään. Ottaa vastaan myös snapshot-kentät ja välittää ne mallille.
 * Snapshot on vapaaehtoinen; jos kenttiä ei tule, riville jää NULL-arvoja.
 */
const addShowTimeForGroup = async (req, res, next) => {
  try {
    ...
    if (!showTimeId || !Number.isFinite(showTimeId)) {
      return next(new ApiError('No show time id', 400))
    }

    // Poimitaan snapshot-kentät jos frontti lähettää ne
    const snapshot = {
      start: req.body?.start ?? null,
      title: req.body?.title ?? null,
      theatre: req.body?.theatre ?? null,
      auditorium: req.body?.auditorium ?? null,
      image: req.body?.image ?? null,
    }

    const result = await addShowTime(userId, groupId, showTimeId, snapshot)
    if (!result) return next(new ApiError('Users is not a member of the group', 403))
    if (result === 'Show time already in group')
      return next(new ApiError('Show time already in group', 400))
